  ...
  db.findOne({
    ...
    "objectId":objectId
       ...
       // var dataObject= JSON.parse(data);
        ...
        var expireDate = createdAt.add(_day, 'days').add(_month, 'months');
        //response.end("createdAt = " + createdAt.format('YYYY-MM-DD HH:mm:ss') );
        // response.end("expireDate = " + expireDate.format('YYYY-MM-DD HH:mm:ss') + ",_day = " + _day + ",_month = " + month);
        if (expireDate >= moment()) { //有效期内
           ...
           // response.end("createdAt = " + createdAt.format('YYYY-MM-DD HH:mm:ss') + ", day = " + day + ",month = " + month);
            ...
            var diff = moment(moment()).diff(moment(expireDate), 'days'); //已经过期了，需要补全
            //response.end("need append diff days = " + diff);
            _day += diff + Number(day);
            _month += Number(month);
           // response.end("need append diff days = " + diff + ",now month = " + _month + ",now day = " + _day);
        }
