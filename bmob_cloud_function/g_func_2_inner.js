/*Google g_func_2的替代函数，增加checkwhite的调用，适配白名单情况（多设备一个账号的情况）
 */ 
            ...
            if (JSON.stringify(resultArr) == '[]') {
                ...
                if (dataObject.banned || dataObject.checkResult == '002') {
                 ...
                 if (dataObject.utdid !== utdid) { //id has been changed
                         functions.run({
                          "name": "g_check_white_state",
                          "data":{"utdid":utdid}
                        },function(err,data){
                              var dataRes= JSON.parse(data);
                            //  response.send(Buffer.from(JSON.stringify(dataRes),'utf-8'));
                            //  response.send(Buffer.from(JSON.stringify(dataObject),'utf-8').toString('base64'));
                               if (dataRes.code === "A102") { //存在白名单且有效,这种情况通常是多个设备，多余的设备使用Paypal支付
                                       _result.code = "A1024";
                                       _result.updatedAt = dataObject.serviceCodeUpdate;
                                       response.send(_result);   
                                       return;
                                 }else { //其他情况，设备不再白名单或者已经过期
